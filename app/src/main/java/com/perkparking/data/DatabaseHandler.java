...
import com.perkparking.model.Category;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private static final String KEY_WORKINGDATE = "workingdate";
    ...
    private static final String KEY_LOTDETAIL   = "lotdetail";
    private static final String KEY_ENVIRONMENT = "environment";
    private static final String KEY_PARKINGLOT  = "parkinglot";
    private static final String KEY_SOVERNIGHT  = "sovernight";
    private static final String KEY_SSECURITY   = "ssecurity";
    private static final String KEY_SDISABLE    = "sdisable";
    private static final String KEY_SLIFT       = "slift";
    private static final String KEY_STOILET     = "stoilet";
    private static final String KEY_CALTIME     = "caltime";
    private static final String KEY_CALPRICE    = "calprice";
    ...
    private ContentValues getPlaceValue(Place model){
        ...
        values.put(KEY_WORKINGDATE, model.workingdate);
        values.put(KEY_WORKINGTIME, model.workingtime);
        values.put(KEY_PRICE, model.price);
        values.put(KEY_PROMOTION, model.promotion);
        values.put(KEY_TERM, model.term);
        values.put(KEY_LOTDETAIL, model.lotdetail);
        values.put(KEY_ENVIRONMENT, model.environment);
        values.put(KEY_PARKINGLOT, model.parkinglot);
        values.put(KEY_SOVERNIGHT, model.sovernight);
        values.put(KEY_SSECURITY, model.ssecurity);
        values.put(KEY_SDISABLE, model.sdisable);
        values.put(KEY_SLIFT, model.slift);
        values.put(KEY_STOILET, model.stoilet);
        values.put(KEY_CALTIME, model.caltime);
        values.put(KEY_CALPRICE, model.calprice);
    ...
    public List<Place> getAllPlace() {
        return getAllPlaceByCategory(0);
    }

    public List<Place> getPlacesByPage(int c_id, int limit, int offset) {
        List<Place> locList = new ArrayList<>();
        StringBuilder sb = new StringBuilder();
        sb.append(" SELECT DISTINCT p.* FROM "+TABLE_PLACE+" p, "+TABLE_PLACE_CATEGORY+" pc ");
        if(c_id == -2) {
    ...
    private Place getPlaceByCursor(Cursor cur){
        ...
        p.email       = cur.getString(cur.getColumnIndex(KEY_EMAIL));
        p.website     = cur.getString(cur.getColumnIndex(KEY_WEBSITE));
        p.workingdate = cur.getString(cur.getColumnIndex(KEY_WORKINGDATE));
        p.workingtime = cur.getString(cur.getColumnIndex(KEY_WORKINGTIME));
        p.price       = cur.getString(cur.getColumnIndex(KEY_PRICE));
        p.promotion   = cur.getString(cur.getColumnIndex(KEY_PROMOTION));
        p.term        = cur.getString(cur.getColumnIndex(KEY_TERM));
        p.lotdetail   = cur.getString(cur.getColumnIndex(KEY_LOTDETAIL));
        p.environment = cur.getString(cur.getColumnIndex(KEY_ENVIRONMENT));
        p.parkinglot  = cur.getString(cur.getColumnIndex(KEY_PARKINGLOT));
        p.sovernight  = cur.getString(cur.getColumnIndex(KEY_SOVERNIGHT));
        p.ssecurity   = cur.getString(cur.getColumnIndex(KEY_SSECURITY));
        p.sdisable    = cur.getString(cur.getColumnIndex(KEY_SDISABLE));
        p.slift       = cur.getString(cur.getColumnIndex(KEY_SLIFT));
        p.stoilet     = cur.getString(cur.getColumnIndex(KEY_STOILET));
        p.caltime     = cur.getInt(cur.getColumnIndex(KEY_CALTIME));
        p.calprice    = cur.getInt(cur.getColumnIndex(KEY_CALPRICE));
    ...
    public int getPlacesSize(int c_id) {
        StringBuilder sb = new StringBuilder();
        sb.append(" SELECT COUNT(DISTINCT p."+KEY_PLACE_ID+") FROM "+TABLE_PLACE+" p, "+TABLE_PLACE_CATEGORY+" pc , "+TABLE_FAVORITES+" f ");
        if(c_id == -2) {
            sb.append(" WHERE p." +KEY_PLACE_ID+ " = f." +KEY_PLACE_ID+" ");
        } else if(c_id != -1){
            sb.append(" WHERE pc." +KEY_RELATION_PLACE_ID+ " = p." +KEY_PLACE_ID+ " AND pc." +KEY_RELATION_CAT_ID+ "=" +c_id+ " ");
